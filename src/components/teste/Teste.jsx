...
const Teste = () => {
  ...
  return (
    <header ref={container}>
        ...
        <div className="z-10 w-full bg-[#01090D80] px-8 py-4 fixed backdrop-blur-2xl border-b-1 border-b-[#E6F7FF40] grid grid-cols-12 gap-8">
            ...
            <button onClick={() => setIsMenuOpen(!isMenuOpen)} className="col-span-1 text-lg text-background self-center flex items-center justify-between cursor-pointer py-2">
              <div className="w-1/2 flex flex-col h-6 overflow-hidden">
                <span className={`will-change-transform duration-750 ease-[cubic-bezier(0.87,0,0.13,1)] ${isMenuOpen ? 'translate-y-[-30px]' : 'translate-y-0'}`}>Menu</span>
                <span className={`will-change-transform duration-750 ease-[cubic-bezier(0.87,0,0.13,1)] ${isMenuOpen ? 'translate-y-[-30px]' : 'translate-y-[10px]'}`}>Fechar</span>
              </div>
              <div className="w-1/2 relative flex flex-col justify-center items-end">
                <span className={`h-[2px] rounded-xl bg-background w-12 absolute transition-all origin-center will-change-transform duration-750 ease-[cubic-bezier(0.87,0,0.13,1)] ${isMenuOpen ? 'translate-y-0 rotate-45 scale-x-[1.05]' : 'translate-y-[-8px]'}`}></span>
                <span className={`h-[2px] rounded-xl bg-background w-12 absolute transition-all origin-center will-change-transform duration-750 ease-[cubic-bezier(0.87,0,0.13,1)] ${isMenuOpen ? 'translate-y-0 rotate-[-45deg] scale-x-[1.05]' : 'translate-y-[8px]'}`}></span>
              </div>
            </button>
        </div>
        ...
        <div ref={overlay} className="fixed top-0 left-0 w-full h-full bg-darkp z-5 clip-overlay">
            <div className="grid grid-cols-12 gap-8 px-8">
                ...
                <div className="col-start-7 col-span-2 flex flex-col justify-end h-dvh pb-16 gap-30">
                    <nav>
                        {menuLinks.map((link, index) => (
                            <div className="w-max overflow-hidden h-24" key={`${link.path}-${index}`}>
                                <div ref={el => menuHolder.current[index] = el} className="relative" onClick={toggleMenu}>
                                    <Link href={link.path} className='text-7xl text-background mt-4'>
                                        {link.label}
                                    </Link>
                                </div>
                            </div>
