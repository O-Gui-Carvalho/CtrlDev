...
const socialLinks = [
    { href: 'https://instagram.com/destaque', label: 'Instagram' },
    { href: 'https://behance.com/destaque', label: 'Behance' },
    { href: 'https://github.com/destaque', label: 'Github' }
]
...
const Header = () => {
    ...
    useGSAP(() => {
        splitInstance.current.forEach(split => split?.revert())
        splitInstance.current = []

        const splitAnim = animTexts.current.map(el => new SplitText(el, { type: 'lines', mask:'lines'}))
        
        splitInstance.current = splitAnim
        ...
        .to(overlay.current, {
            duration: configs.menuDuration,
            clipPath: "polygon(0% 0%, 100% 0%, 100% 100%, 0% 100%)",
            ease: configs.ease,
        })
        .to(items, {
            y: 0,
            duration: configs.itemsDuration,
        ...
        <div className="z-10 w-full bg-[#01090D80] fixed backdrop-blur-2xl border-b border-b-[#E6F7FF80]">
            <div className="custom-grid py-4">
                ...
                <div className="col-span-1 flex items-center justify-end">
                    <button 
                        onClick={toggleMenu} 
                        className="w-12 text-lg text-background self-center cursor-pointer"
        ...
        >
            <div className="grid grid-rows-3 grid-cols-4 md:grid-cols-8 lg:grid-cols-12 md:gap-4 lg:gap-8 px-8 h-dvh content-end">
                ...
                <div className="col-start-1 md:row-span-2 md:col-start-7 lg:col-start-10 col-span-2 flex flex-col justify-end overflow-hidden">
                    <p 
                        className="text-[#ffffff80] text-[clamp(0.75rem,2vw,1rem)] pb-16 md:pb-0" 
                        ref={el => animTexts.current[0] = el}
                    >
                ...
                <div className="col-start-1 md:col-start-5 lg:col-start-7 col-span-2 flex flex-col justify-end gap-8 pb-8">
                    <span 
                        className="text-[#ffffff80] text-[clamp(0.75rem,2vw,1rem)]" 
                        ref={el => animTexts.current[1] = el}
                    >
                        Espirito Santo
                    </span>
                </div>
                        ...
                        <ul className="list-none space-y-2">
                            {socialLinks.map((social, index) => (
                                <li key={social.href}>
                                    <a 
                                        href={social.href}
                                        ref={el => animTexts.current[index + 2] = el}
                                        target='_blank'
