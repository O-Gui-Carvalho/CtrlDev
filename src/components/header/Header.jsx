        ...
        <div className="z-10 w-dvw bg-[#01090D80] px-2 md:px-4 lg:px-8 py-4 fixed backdrop-blur-2xl border-b-1 border-b-[#E6F7FF80] grid gap-8 grid-cols-4 md:grid-cols-8 lg:grid-cols-12">
            <div className="justify-between items-start col-span-3 md:col-span-6 lg:col-span-10">
                <div className="max-w-fit h-full flex items-center">
                    <Link href='/'>
                        <svg viewBox="0 0 126 33" fill="none" xmlns="http://www.w3.org/2000/svg" className="w-24 h-auto md:w-28 lg:w-32" >
            ...
            <div className="col-span-1 flex items-center justify-end">
                <button onClick={() => setIsMenuOpen(!isMenuOpen)} className="w-12 text-lg text-background self-center cursor-pointer">
                    <div className="relative flex flex-col justify-center items-end py-4">
                        <span className={`h-[3px] rounded-xl bg-background w-8 md:w-12 absolute transition-all origin-center will-change-transform duration-750 ease-[cubic-bezier(0.87,0,0.13,1)] ${isMenuOpen ? 'translate-y-0 rotate-45 scale-x-[1.05]' : 'translate-y-[-8px]'}`}></span>
                        <span className={`h-[3px] rounded-xl bg-background w-8 md:w-12 absolute transition-all origin-center will-change-transform duration-750 ease-[cubic-bezier(0.87,0,0.13,1)] ${isMenuOpen ? 'translate-y-0 rotate-[-45deg] scale-x-[1.05]' : 'translate-y-[8px]'}`}></span>
                    </div>
                </button>
            </div>
        </div>
